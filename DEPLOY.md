...
2. Đặt tên: `kinkin-pgh`, region **Singapore (ap-southeast-1)** (gần VN nhất).
...
vercel link
```
Trả lời: Set up `c:\VScode\Kinkin - DF`? → **Y**, scope → personal, link tới existing project → **N**, project name → `kinkin-pgh`, root directory → `.`
...
Sau ~1 phút sẽ có URL: `https://kinkin-pgh.vercel.app`
